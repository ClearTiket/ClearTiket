...
YAML_FILE_PATH = Path.joinpath(BASE_DIR.parent, "config.yml")
...
# config.yml 파일이 프로젝트 루트 경로에 없다면 오류를 발생시킴
if not Path.exists(YAML_FILE_PATH):
    raise FileNotFoundError(
        f"'config.yml' 파일을 찾을 수 없음.\n"
        f"'config.yml' 파일이 프로젝트 루트 경로에 있는지 확인"
    )
...
with open(YAML_FILE_PATH, "r", encoding="utf-8") as f:
    try:
        ...
        raise ValueError(f"config.yml 파일의 내용을 읽을 수 없음: {exc}")
